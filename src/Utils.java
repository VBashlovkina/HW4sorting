...
class Utils
{
   ...
   *   Loop invariant:
   *   
   *   Let current be the index in the result array;
   *   j -- in the a1;
   *   k -- in the a2;
   *   
   *   For all j, k and current, if j is in bounds (lb1 <= j < ub1) and 
   *   a1[j] <= a2[k], or if j is out of bounds (k > ub2),
   *   then result[current] = a1[j].
   *   
   *   For all j, k and current, if k is in bounds (lb2 <= k < ub2) and 
   *   a1[j] >= a2[k], or if k is out of bounds (j > ub1),
   *   then result[current] = a2[k].
   *   
   *   For all 0 <= current < (ub1-lb1) + (ub2-lb2) -1, sorted(result, order, 0, current)
   */
                              ...
                              T[] a2, int lb2, int ub2)
  {
    ...
    while (current < (ub1 - lb1) + (ub2 - lb2))
      {
        if ((j < ub1 && order.compare(a1[j], a2[k]) < 0) || k > ub2)
          {
            result[current] = a1[j++];
            current++;
          }
  ...
  {
    Comparator<Integer> comparator = new StandardIntegerComparator();
    Integer[] ints1 = { new Integer(1), new Integer(3) };
    Integer[] ints2 = { new Integer(2), new Integer(4) };
    Integer[] result = (Integer[]) Utils.merge(comparator, ints1, ints2);
