      ...
      SorterBridge
{
   ...
   * vals such that elements smaller than the pivot appear to the left, elements
   * bigger than the pivot appear to the right of the pivot, and the pivot is in
   * the middle.
   * 
   * @param values
   *          , an array.
   ...
   * @post For all i, lb <= i < mid, order.compare(values[i],pivot) <= 0 For all
   *       i, mid < i < ub, order.compare(pivot, values[i]) < 0
   * 
   *       Loop invariants: All items below the lower bound are smaller than the
