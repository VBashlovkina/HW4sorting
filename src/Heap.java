...
/**
 * A simple implementation of the heap data structure. Reveals some aspects of
 * the heap implementation so that other classes can use it (e.g., for heap
 * sort).
 * 
...
public class Heap<T>
{
  ...
  /*
   * When we refer to an array (or a subarray) of size size as a "heap", we mean
   * that it has the heap property. For all i, 0 <= i < size: if (left(i) <
   * size) order.compare(heap[i], heap[left(i)]) >= 0. if (right(i) < size)
   ...
   * 
   * @pre pos >= 0
   * @pre The first pos elements of heap have the heap property.
   * @pre heap[pos] can be compared to other values using the comparator
   * @post The first pos+1 elements of heap have the heap property.
   */
  ...
  /**
   * Swap down iteratively
   * 
   * @pre 0 <= pos < size
   * @post heap has the heap property
   */
  public static <T> void swapItDown(T[] heap, int size, int pos,
                                    Comparator<T> order)
  {
    // left child
    int l;
    ...
    while (true)
      {
        l = left(pos); // Index of the left child.
        r = right(pos); // Index of the right child;
        if (l >= size)
          {
            ...
            if (order.compare(heap[l], heap[r]) >= 0)
              {
                ...
                if (order.compare(heap[pos], heap[l]) < 0)
                  ...
                  {
                    return;
                  }// else current element is in the right place
  ...
  }// swapItDown (T[], int, int, Comparator<T>)

  /**
   * Restore the heap property by propagating down from pos.
   * 
   * @pre pos < size
   * @pre The first pos elements of heap have the heap proprety.
   * @pre All indices i, pos < i < size, have the heap property
   * @post The first size elements of heap have the heap property.
   */
  ...
  /**
   * Create a heap with the specified initial capacity that uses the given
   * comparator to order values.
   */
  ...
  /**
   * Create a heap with the default initial capacity that uses the given
   * comparator to order values.
   */
    ...
    throws Exception
  {
    ...
    Heap.swapItDown(this.values, this.size, 0, this.order);
    // And we're done
    return result;
  } // get()
  ...
  /**
   * Print the standard tree representation of this heap, starting a position
   * pos, indenting values by indent.
   */
